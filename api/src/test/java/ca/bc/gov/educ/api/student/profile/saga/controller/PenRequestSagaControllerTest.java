...
public class PenRequestSagaControllerTest extends BaseSagaApiTest {
  ...
  public void test_returnPENRequest_givenValidPayload_shouldReturnStatusOK() throws Exception {
    ...
    this.mockMvc.perform(post(URL.BASE_URL + URL.PEN_REQUEST_RETURN_SAGA).with(jwt().jwt((jwt) -> jwt.claim("scope", "PEN_REQUEST_RETURN_SAGA"))).contentType(MediaType.APPLICATION_JSON).content(payload)).andDo(print()).andExpect(status().isOk());
    val result = this.repository.findAll();
    assertEquals(1, result.size());
  }
